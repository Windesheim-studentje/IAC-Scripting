...
function welkom {
    echo "Welkom bij deployment script voor het uitrollen van virtuele machines."
    echo "Voer uw klantnaam in (geen spaties):"
    read klantnaam
    klantdir="./${klantnaam}-prod"  # Aangepast pad naar klantmap
    mkdir -p "${klantdir}"
    echo "Nieuwe SSH key genereren voor deze klantomgeving."
    ssh-keygen -t rsa -b 4096 -N "" -f "${klantdir}/id_rsa"
    
    # Rechten instellen
    chmod 600 "${klantdir}/id_rsa"
    chmod 644 "${klantdir}/id_rsa.pub"
    
    # Kopieer de private sleutel naar de lokale SSH-directory
    cp "${klantdir}/id_rsa" /home/student/.ssh/id_rsa
    chmod 600 /home/student/.ssh/id_rsa
    
    # Kopieer de publieke sleutel naar de lokale SSH-directory
    cp "${klantdir}/id_rsa.pub" /home/student/.ssh/id_rsa.pub
...
function genereer_vagrantfile {
    ...
    echo "Controleer of de SSH-sleutel bestaat: ${klantdir}/id_rsa"
    if [ ! -f "${klantdir}/id_rsa" ]; then
        echo "Fout: De SSH-sleutel ${klantdir}/id_rsa bestaat niet."
        exit 1
    fi

    # Dynamisch absoluut pad
    full_path="${klantdir}/id_rsa"
...
EOF
    ...
    for (( i=1; i<=$aantal_machines; i++ )); do
        vm_name="${klantnaam}-${servertype,,}-productie$i"
...
function start_deployment {
    echo "Vagrantfile + Ansible Playbook voor type $servertype zijn in de map ${klantnaam}-productie voor $aantal machines machines gedeployed."
